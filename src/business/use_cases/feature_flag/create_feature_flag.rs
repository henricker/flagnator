...
use crate::{business::repositories::{project_repository::ProjectRepository, feature_flag_repository::FeatureFlagRepository}, domain::entities::feature_flag::FeatureFlag};
...
impl<'a> CreateFeatureFlagUseCase<'a> {
    pub fn new(project_repository: &'a dyn ProjectRepository, feature_flag_repository: &'a dyn FeatureFlagRepository) -> Self {
    ...
    pub fn execute(&self, name: String, project_id: Uuid, organization_id: Uuid) -> Result<FeatureFlag, Box<dyn Error>> {
        ...
        let feature_name_exists = self.feature_flag_repository.name_exists(&name, project_id)?;
...
mod tests {
    ...
    use crate::{business::repositories::{project_repository::MockProjectRepository, feature_flag_repository::MockFeatureFlagRepository}, domain::entities::project::Project};
    ...
    fn test_create_project_if_project_not_exists() {
        ...
        mock_project_repo.expect_get().with(eq(project_uuid)).times(1).returning(|_| Ok(None));
        ...
        match *result.unwrap_err().downcast::<CreateFeatureFlagError>().unwrap() {
    ...
    fn test_create_project_if_project_not_belongs_organization() {
        ...
        mock_project_repo.expect_get().with(eq(project_uuid)).times(1).returning(move |_| Ok(Some(
            Project::new(project_uuid, other_organization_uuid, "feature-x".to_string(), Vec::new())
        )));
    ...
    fn test_create_project_if_feature_name_already_exists_in_project() {
        ...
        mock_project_repo.expect_get().with(eq(project_uuid)).times(1).returning(move |_| Ok(Some(
            Project::new(project_uuid, organization_uuid, "feature-x".to_string(), Vec::new())
        )));

        mock_feature_flag_repo.expect_name_exists().with(eq("feature-x"), eq(project_uuid)).returning(|_, _| Ok(true));
    ...
    fn test_create_project_if_feature_on_success() {
        ...
        mock_feature_flag_repo.expect_name_exists().with(eq("feature-x"), eq(project_uuid)).returning(|_, _| Ok(false));
        mock_feature_flag_repo.expect_add().returning(|_| Ok(()));
        let use_case = CreateFeatureFlagUseCase::new(&mock_project_repo, &mock_feature_flag_repo);
        
        // Act
        let result = use_case.execute("feature-x".to_string(), project_uuid, organization_uuid).unwrap();
