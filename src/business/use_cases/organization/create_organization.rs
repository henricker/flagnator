...
use crate::{domain::entities::organization::Organization, business::services::hash_service::HashService};
...
impl<'a> CreateOrganizationUseCase<'a> {
    pub fn new(repository: &'a dyn OrganizationRepository, hash_service: &'a dyn HashService) -> Self {
    ...
    pub fn execute(&self, name: String, email: String, password: String) -> Result<Organization, Box<dyn Error>> {
...
mod tests {
    ...
    use crate::business::{repositories::organization_repository::MockOrganizationRepository, services::hash_service::MockHashService};
    ...
    fn test_create_organization_success() {
        ...
        mock_organization_repo.expect_add()
                 .times(1)
                 .returning(|_| Ok(()));

        mock_hash_service.expect_hash()
                .with(eq("password123"))
                .times(1)
    ...
    fn test_create_organization_email_exists() {
        ...
        mock_organization_repo.expect_email_exists()
        ...
        match *result.unwrap_err().downcast::<CreateOrganizationError>().unwrap() {
